...
$company_condition = db_quote('(?:promotions.company_id IN (?n) OR ?:promotions.company_id = ?i)', Tygh::$app['storefront']->getCompanyIds(), 0);
} else {
$company_condition = 1;
}
if (!empty($params['dispatch']) && $params['dispatch'] === 'promotions.picker' && $params['display'] === 'radio') {
$search = db_quote('?:promotions.company_id = 0');
$condition = str_replace($search, $company_condition, $condition);
} elseif (AREA === 'C') {
$condition .= db_quote(' AND ?p', $company_condition);
}
}
if (in_array('ab__dotd_get_seo_data', $params['extend'])) {
$join .= ' LEFT JOIN ?:ab__dotd ON ?:ab__dotd.promotion_id = ?:promotions.promotion_id';
$fields[] = '?:ab__dotd.*';
}
}
...
$products = db_get_fields('SELECT p.product_id FROM ?:products AS p WHERE ?p', $amount_condition);
foreach ($chains as $chain) {
...
$promotion = fn_ab__dotd_get_cached_promotion_data($params['ab__dotd_promotion_id']);
if (isset($promotion['conditions'])) {
list($ab__dotd_where, $ab__dotd_joins) = fn_ab__dotd_build_promotion_conditions_query($promotion['conditions']);
$join .= db_quote(' ?p', implode(' ', $ab__dotd_joins));
$condition .= db_quote(' AND (?p)', $ab__dotd_where);
}
...
$allowed_storefronts = explode(',', $promotion_data['storefront_ids']);
$storefront_id = Tygh::$app['storefront']->storefront_id;
$promotion_data = in_array($storefront_id, $allowed_storefronts) ? $promotion_data : [];
}

if (fn_allowed_for('MULTIVENDOR') && !empty($promotion_data['company_id'])) {
$company_ids = Tygh::$app['storefront']->getCompanyIds();
if (!empty($company_ids) && !in_array($promotion_data['company_id'], $company_ids)) {
$promotion_data = [];
}
}
}
...
if ($params['exclude_hidden'] && $promotion['status'] === 'H') {
continue;
}
...
{
$plurals = [];
$lang_vars = ['seconds', 'minutes', 'hours', 'days'];
