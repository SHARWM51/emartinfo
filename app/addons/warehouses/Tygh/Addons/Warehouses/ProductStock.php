...
class ProductStock
{
     ...
     *
     * @param string $change_stock_by Product amount
     * @param int    $amount          Product amount
     * @param int    $pickup_point_id Pickup point identifier from order
     * @param int    $destination_id  Destination identifier from order
     *
     * @return $this|null
     */
    public function reduceAmount($change_stock_by, $amount, $pickup_point_id = 0, $destination_id = 0)
    {
        if ($change_stock_by === 'store') {
            return $this->reduceStockByAmountForStore($amount, $pickup_point_id);
        }
        if ($change_stock_by === 'destination') {
            return $this->reduceStockByAmountForDestination($amount, $destination_id);
        }
        if ($change_stock_by === 'amount') {
     ...
     * @return $this|null
     */
    public function increaseAmount($change_stock_by, $amount, $pickup_point_id = 0, $destination_id = 0)
    {
        if ($change_stock_by === 'store') {
