...
function fn_warehouses_get_products(array &$params, array &$fields, array &$sortings, &$condition, &$join, $sorting, $group_by, $lang_code, $having)
{
    ...
    $is_need_join = $check_warehouse_product_amount || !empty($params['destination_ids']);
    ...
    if (!$sort_products_by_amount && empty($params['destination_ids'])) {
        return;
    }
...
function fn_warehouses_update_product_amount(&$new_amount, $product_id, $cart_id, $tracking, $notify, $order_info, $amount_delta, $current_amount, $original_amount, $sign)
{
    ...
    if ($sign === '-') {
        $product_stock->reduceAmount($change_stock_by, $amount_delta, $pickup_point_id, $destination_id);
    } else {
...
function fn_warehouses_master_products_get_best_product_offer_pre(&$master_product_id, array &$vendor_product_ids)
{
    ...
    $join = db_quote(
        ' LEFT JOIN ?:warehouses_products_amount'
        . ' AS war_amounts'
