...
{hook name="products:product_name"}
    ...
    {elseif $show_trunc_name}
        {if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product|truncate:160:"...":true nofilter}{if $hide_links}</strong>{else}</a>{/if}
    {/if}
{/hook}
{/capture}
{if $no_capture}
...
{capture name="buttons_product"}
    {hook name="products:add_to_cart"}
        {if $product.has_options && !$show_product_options && !$details_page}
            ...
            {include file="buttons/button.tpl" but_id="button_cart_`$obj_prefix``$obj_id`" but_text=__("select_options") but_href="products.view?product_id=`$product.product_id`" but_role=$opt_but_role but_name="" but_meta="ut2-btn__options ty-btn__primary ty-btn__big"}
        {else}
            ...
            {include file="buttons/add_to_cart.tpl" but_id=$_but_id but_name="dispatch[checkout.add..`$product.product_id`]" but_role=$but_role block_width=$block_width obj_id=$obj_id product=$product but_meta=$add_to_cart_meta}
...
{hook name="products:buttons_block"}
    ...
    {elseif ($settings.General.inventory_tracking !== "YesNo::NO"|enum && $settings.General.allow_negative_amount != "YesNo::YES"|enum && (($product_amount <= 0 || $product_amount < $product.min_qty) && $product.tracking != "ProductTracking::DO_NOT_TRACK"|enum) && $product.is_edp != "YesNo::YES"|enum)}
        {hook name="products:out_of_stock_block"}
            {assign var="show_qty" value=false}
            {if !$details_page}
                {if $settings.abt__ut2.product_list.$tmpl.show_amount[$settings.abt__device] == "YesNo::NO"|enum && (!$product.hide_stock_info && !(($product_amount <= 0 || $product_amount < $product.min_qty) && ($product.avail_since > $smarty.const.TIME)))}
            ...
            {elseif ($product.out_of_stock_actions == "OutOfStockActions::SUBSCRIBE"|enum)}
                ...
                <div class="ty-control-group">
                    <label for="sw_product_notify_{$obj_prefix}{$obj_id}" class="ty-strong" id="label_sw_product_notify_{$obj_prefix}{$obj_id}">
                        <input id="sw_product_notify_{$obj_prefix}{$obj_id}" type="checkbox" class="checkbox cm-switch-availability cm-switch-visibility" name="product_notify" {if $product_notification_enabled == "YesNo::YES"|enum}checked="checked"{/if} onclick="
                            {if !$auth.user_id}
                                if (!this.checked) {
...
{capture name="prod_descr_`$obj_id`"}
    {if $show_descr}
        {if $product.short_description}
            <div {live_edit name="product:short_description:{$product.product_id}"}>{$product.short_description|strip_tags nofilter}</div>
        {else}
            <div {live_edit name="product:full_description:{$product.product_id}" phrase=$product.full_description}>{$product.full_description|strip_tags|truncate:300 nofilter}</div>
        {/if}
    {/if}
{/capture}
{if $no_capture}
...
{capture name="clean_price_`$obj_id`"}
    {if $show_price_values && $show_clean_price && $settings.Appearance.show_prices_taxed_clean == "YesNo::YES"|enum && $product.taxed_price}
            ...
            {if $settings.Appearance.in_stock_field == "YesNo::YES"|enum}
                {if $product.tracking != "ProductTracking::DO_NOT_TRACK"|enum}
                    {if ($product_amount > 0 && $product_amount >= $product.min_qty) && $settings.General.inventory_tracking !== "YesNo::NO"|enum || $details_page}
                        ...
                        {elseif $settings.General.inventory_tracking !== "YesNo::NO"|enum && $settings.General.allow_negative_amount != "YesNo::YES"|enum}
...
{capture name="product_options_`$obj_id`"}
    ...
    <div class="cm-reload-{$obj_prefix}{$obj_id} js-product-options-{$obj_prefix}{$obj_id}" id="product_options_update_{$obj_prefix}{$obj_id}">
        ...
        {hook name="products:product_option_content"}
            ...
            {/if}
            {include file="views/products/components/product_options.tpl" id=$product.product_id product_options=$product.product_options name="product_data" capture_options_vs_qty=$capture_options_vs_qty disable_ids=$_disable_ids}
        {/hook}
    ...
    {hook name="products:qty"}
        ...
        {if $show_qty && $product.is_edp !== "YesNo::YES"|enum && $cart_button_exists == true && ($settings.Checkout.allow_anonymous_shopping == "allow_shopping" || $auth.user_id) && $product.avail_since <= $smarty.const.TIME || ($product.avail_since > $smarty.const.TIME && $product.out_of_stock_actions == "OutOfStockActions::BUY_IN_ADVANCE"|enum)}
            <div class="ty-qty clearfix{if $settings.Appearance.quantity_changer == "YesNo::YES"|enum} changer{/if}" id="qty_{$obj_prefix}{$obj_id}">
                ...
                <select name="product_data[{$product.product_id}][amount]" id="qty_count_{$obj_prefix}{$obj_id}">
        ...
        {elseif !$bulk_add}
            <input type="hidden" name="product_data[{$product.product_id}][amount]" value="{$default_amount}" />
        {/if}
...
{capture name="product_edp_`$obj_id`"}
    {if $show_edp && $product.is_edp == "YesNo::YES"|enum}
        ...
        <input type="hidden" name="product_data[{$product.product_id}][is_edp]" value="{"YesNo::YES"|enum}" />
    {/if}
{/capture}
{if $no_capture}
