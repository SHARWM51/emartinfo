...
{if $allow_new_thread}
    ...
    {else}
        ...
        <a title="{$title}" href="{"auth.login_form?return_url=`$return_current_url`"|fn_url}" data-ca-target-id="new_thread_login_form" class="vc__l cm-dialog-opener cm-dialog-auto-size{if "MULTIVENDOR"|fn_allowed_for} ty-vendor-communication__post-write{/if}" rel="nofollow">
            <i class="ut2-icon-outline-announcement"></i>
            <span class="ajx-link">{$title}</span>
        </a>
